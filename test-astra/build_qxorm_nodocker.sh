#!/bin/bash
# build_qxorm_nodocker.sh — собрать пакет qxorm/1.5.1 БЕЗ docker-self-wrap.
#
...
#
# Версия 1.5.1 = upstream latest (легаси-пина нет: QXORM на фото не вскрыт).
#
# export -> build Release+Debug -> deployer -> qxorm.<...>.nupkg.
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ./test-astra/build_qxorm_nodocker.sh              # x86_64 (единственный)
#
# Env:
#   QXORM_VERSION   версия qxorm (по умолч. 1.5.1)
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-qxorm-x86_64)
...
QXORM_VERSION="${QXORM_VERSION:-${PKG_VERSION:-1.5.1}}"
ARCH="${ARCH:-x86_64}"
if [ "$ARCH" != "x86_64" ]; then
    echo "[FAIL] qxorm собирается только на x86_64 (Qt 5.15.2 есть лишь в образе gcc84-build-x86_64; arm/arm64-образы Qt не несут)" >&2
    exit 2
fi
PROFILE="${PROFILE:-profiles/lin-gcc84-x86_64}"
PROFILE_BUILD="${PROFILE_BUILD:-$PROFILE}"
OUTPUT_DIR="${OUTPUT_DIR:-output-qxorm-x86_64}"
SHARED="${SHARED:-False}"
...
conan export qxorm/ --version="$QXORM_VERSION" --no-remote
