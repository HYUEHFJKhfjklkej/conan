#!/bin/bash
# build_jsoncpp_nodocker.sh — собрать пакет jsoncpp/1.9.8 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> jsoncpp.<...>.nupkg. Никакого
...
# jsoncpp — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_jsoncpp_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_jsoncpp_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_jsoncpp_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   JSONCPP_VERSION   версия jsoncpp (по умолч. 1.9.8)
#   PKG_VERSION   generic-фолбэк версии (если JSONCPP_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-jsoncpp-<arch>)
...
JSONCPP_VERSION="${JSONCPP_VERSION:-${PKG_VERSION:-1.9.8}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-jsoncpp-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт jsoncpp читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта jsoncpp.
conan export jsoncpp/ --version="$JSONCPP_VERSION" --no-remote
