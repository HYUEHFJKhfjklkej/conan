#!/bin/bash
# build_gtest_nodocker.sh — собрать пакет gtest/1.17.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> googletest.<...>.nupkg
# (deployer переименовывает gtest -> googletest, LEGACY_NAME_MAP). Никакого
# docker build/pull/run — контейнер-станок поднимает сам TeamCity
# ("Run step within Docker container"), как у fmt/grpc-драйверов. gtest —
# одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_gtest_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_gtest_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_gtest_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   GTEST_VERSION версия gtest (по умолч. 1.17.0 — последняя upstream)
#   PKG_VERSION   generic-фолбэк версии (если GTEST_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-gtest-<arch>)
...
GTEST_VERSION="${GTEST_VERSION:-${PKG_VERSION:-1.17.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-gtest-$ARCH}"
SHARED="${SHARED:-False}"
...
# Кросс-тулчейн для arm/arm64 gtest берёт из [conf] *:user_toolchain самого
# host-профиля (gtest собирается напрямую, не транзитив — env-fallback как в
# grpc-дереве не нужен). Экспорт CONAN_USER_TOOLCHAIN оставлен для единообразия
# с другими драйверами и безвреден (рецепт gtest его не читает).
...
# Экспорт рецепта gtest.
conan export gtest/ --version="$GTEST_VERSION" --no-remote
...
# Deployer → legacy .nupkg (gtest -> googletest). Те же опции, что в build.
rm -f "$OUTPUT_DIR"/googletest.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
...
echo "[DONE] $REF ($ARCH) -> $OUTPUT_DIR/ (googletest.*.nupkg)"
