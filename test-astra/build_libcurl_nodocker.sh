#!/bin/bash
# build_libcurl_nodocker.sh — собрать пакет libcurl/8.21.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> libcurl.<...>.nupkg. Никакого
...
# libcurl — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_libcurl_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_libcurl_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_libcurl_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LIBCURL_VERSION   версия libcurl (по умолч. 8.21.0)
#   PKG_VERSION   generic-фолбэк версии (если LIBCURL_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-libcurl-<arch>)
...
LIBCURL_VERSION="${LIBCURL_VERSION:-${PKG_VERSION:-8.21.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-libcurl-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт libcurl читает CONAN_USER_TOOLCHAIN
