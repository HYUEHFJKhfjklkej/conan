#!/bin/bash
# build_expat_nodocker.sh — собрать пакет expat/2.8.2 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> expat.<...>.nupkg. Никакого
...
# expat — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_expat_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_expat_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_expat_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   EXPAT_VERSION   версия expat (по умолч. 2.8.2)
#   PKG_VERSION   generic-фолбэк версии (если EXPAT_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-expat-<arch>)
...
EXPAT_VERSION="${EXPAT_VERSION:-${PKG_VERSION:-2.8.2}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-expat-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт expat читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта expat.
conan export expat/ --version="$EXPAT_VERSION" --no-remote
