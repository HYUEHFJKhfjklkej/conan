#!/bin/bash
# build_mbedtls_nodocker.sh — собрать пакет mbedtls/3.6.6 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> mbedtls.<...>.nupkg. Никакого
...
# mbedtls — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_mbedtls_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_mbedtls_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_mbedtls_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   MBEDTLS_VERSION   версия mbedtls (по умолч. 3.6.6)
#   PKG_VERSION   generic-фолбэк версии (если MBEDTLS_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-mbedtls-<arch>)
...
MBEDTLS_VERSION="${MBEDTLS_VERSION:-${PKG_VERSION:-3.6.6}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-mbedtls-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт mbedtls читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта mbedtls.
conan export zlib/ --version=1.3.1 --no-remote   # dep
conan export mbedtls/ --version="$MBEDTLS_VERSION" --no-remote
