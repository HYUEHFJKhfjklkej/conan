#!/bin/bash
# build_soem_nodocker.sh — собрать пакет soem/2.0.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> soem.<...>.nupkg. Никакого
...
# soem — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_soem_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_soem_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_soem_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   SOEM_VERSION   версия soem (по умолч. 2.0.0)
#   PKG_VERSION   generic-фолбэк версии (если SOEM_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-soem-<arch>)
...
SOEM_VERSION="${SOEM_VERSION:-${PKG_VERSION:-2.0.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-soem-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт soem читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта soem.
conan export soem/ --version="$SOEM_VERSION" --no-remote
