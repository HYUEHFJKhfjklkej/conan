#!/bin/bash
# build_libiec61850_nodocker.sh — собрать пакет libiec61850/1.6.1 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> libiec61850.<...>.nupkg. Никакого
...
# libiec61850 — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_libiec61850_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_libiec61850_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_libiec61850_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LIBIEC61850_VERSION   версия libiec61850 (по умолч. 1.6.1)
#   PKG_VERSION   generic-фолбэк версии (если LIBIEC61850_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-libiec61850-<arch>)
...
LIBIEC61850_VERSION="${LIBIEC61850_VERSION:-${PKG_VERSION:-1.6.1}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-libiec61850-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт libiec61850 читает CONAN_USER_TOOLCHAIN
...
REF="libiec61850/$LIBIEC61850_VERSION"
mkdir -p "$OUTPUT_DIR"
...
# Экспорт рецепта libiec61850.
conan export libiec61850/ --version="$LIBIEC61850_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/libiec61850.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
