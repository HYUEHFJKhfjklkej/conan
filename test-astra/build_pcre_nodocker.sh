#!/bin/bash
# build_pcre_nodocker.sh — собрать пакет pcre/8.45 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> pcre.<...>.nupkg. Никакого
...
# pcre — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_pcre_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_pcre_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_pcre_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   PCRE_VERSION   версия pcre (по умолч. 8.45)
#   PKG_VERSION   generic-фолбэк версии (если PCRE_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-pcre-<arch>)
...
PCRE_VERSION="${PCRE_VERSION:-${PKG_VERSION:-8.45}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-pcre-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт pcre читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта pcre.
conan export pcre/ --version="$PCRE_VERSION" --no-remote
