#!/bin/bash
# build_fmt_nodocker.sh — собрать пакет fmt/11.2.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> fmt.<...>.nupkg. Никакого
...
# fmt — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_fmt_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_fmt_nodocker.sh      # кросс armv7hf
...
OUTPUT_DIR="${OUTPUT_DIR:-output-fmt-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт fmt читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта fmt.
conan export fmt/ --version="$FMT_VERSION" --no-remote
