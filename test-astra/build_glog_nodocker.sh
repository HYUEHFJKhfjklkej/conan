#!/bin/bash
# build_glog_nodocker.sh — собрать пакет glog/0.7.1 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> glog.<...>.nupkg. Никакого
...
# glog — вне grpc-дерева; deps: gflags (with_gflags=True, классическая пара).
# libunwind ВЫКЛЮЧЕН (-o with_unwind=False): conan-депа libunwind у нас нет,
# и в легаси-дереве его не было.
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_glog_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_glog_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_glog_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   GLOG_VERSION   версия glog (по умолч. 0.7.1)
#   PKG_VERSION   generic-фолбэк версии (если GLOG_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-glog-<arch>)
...
GLOG_VERSION="${GLOG_VERSION:-${PKG_VERSION:-0.7.1}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-glog-$ARCH}"
SHARED="${SHARED:-False}"
...
# Кросс-тулчейн CMake рецепт получает из [conf] user_toolchain host-профиля.
# CONAN_USER_TOOLCHAIN экспортируем для deployer'а: суффикс арки `-linaro`
# в имени .nupkg + выбор target-binutils.
...
# Экспорт рецептов glog + gflags (dep).
conan export gflags/ --version=2.3.0 --no-remote
conan export glog/ --version="$GLOG_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/{glog,gflags}.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" -o "glog/*:with_unwind=False" -o "gflags/*:nothreads=False" \
