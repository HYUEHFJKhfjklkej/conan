#!/bin/bash
# ============================================================================
# Оркестратор тестов для всех целевых профилей conan-recipes (IN-658 / IN-353).
#
# Собирает образ Dockerfile.grpc-tc-mirror под нужную архитектуру и прогоняет
# полное дерево grpc + 6 deps, давая 7 .nupkg в TC-совместимом формате. Замена
# старого arm-only test_arm_cross.sh — покрывает все CI-архитектуры:
# x86_64, i686, armv7hf (arm), arm64.
#
# Запуск:
#   ./test_all_profiles.sh <mode> <arch>
#       mode: smoke   — только pre-flight + диагностика образа (~2 мин)
#             build   — полное дерево Conan + проверка .nupkg (~15-90 мин)
#             all     — `build` для x86_64, armv7hf, arm64 подряд
#       arch: x86_64 | i686 | armv7hf | arm | arm64
#                 (arm = алиас armv7hf, работают оба)
#
# Обязательный env:
#   REGISTRY        — внутренний registry, напр. proget.example/main
#                     Пропускается, если задан LOCAL_BASE (mac/dev режим).
#
# Опциональный env:
#   LOCAL_BASE             — полностью переопределяет BASE_IMAGE. Для локального
#                            теста на Mac без внутреннего registry. Пример:
#                              LOCAL_BASE=elara-public-base:latest \
#                                ./test_all_profiles.sh build armv7hf
#   BASE_IMAGE_TAG         — default: 0.1.0 для arm/arm64, latest для x86.
#   CMAKE_BUILD_PARALLEL_LEVEL — default 4 (безопасно при 8GB памяти Docker).
#   SHARED                 — True/False, default False (контент — static .a).
#                             Slot-tag в имени задаёт LEGACY_NUPKG_LINKAGE
#                             (default "shared" — GR113/DynamicRT). См. INFRASTRUCTURE.md §3.7.
#   FORCE_REBUILD          — непусто → снести output dir + conan-cache перед прогоном.
#
# Вывод:
#   ./output-<arch>/conan-cache/   — bind-mount кэш Conan (живёт между прогонами)
#   ./output-<arch>/*.nupkg        — 7 nuget-пакетов
#   ./output-<arch>/run.log        — полный вывод conan
#
# Зачем этот скрипт:
#   - test_arm_cross.sh НЕ пробрасывал -e CONAN_USER_TOOLCHAIN, поэтому env-fallback
#     против бага transitive user_toolchain в Conan 2.27.1 (коммиты de4802b,
#     8278d55, NEXT_STEPS.md 4b) молча превращался в no-op.
#   - test_arm_cross.sh НЕ bind-mount'ил /root/.conan2 на хост, поэтому conan-cache
#     умирал с каждым --rm контейнером — каждая итерация была полной пересборкой.
#   Этот скрипт чинит оба и добавляет x86_64 / i686 / arm64.
# ============================================================================

set -uo pipefail

MODE="${1:-}"
ARCH="${2:-}"

usage() {
    sed -n '2,40p' "$0"
    exit 2
}

[[ -z "$MODE" || -z "$ARCH" ]] && usage
[[ "$MODE" =~ ^(smoke|build|all)$ ]] || { echo "[FAIL] mode must be smoke|build|all, got '$MODE'" >&2; exit 2; }

# Режим 'all' — рекурсивный прогон трёх продакшн-целей.
if [[ "$MODE" == "all" ]]; then
    [[ "$ARCH" != "_" && "$ARCH" != "all" ]] && { echo "[FAIL] for mode=all, arch must be 'all' or '_'" >&2; exit 2; }
    rc=0
    for a in x86_64 armv7hf arm64; do
        echo ""
        echo "########################################"
        echo "# all → $a"
        echo "########################################"
        "$0" build "$a" || rc=1
    done
    exit $rc
fi

# Нормализация arch.
case "$ARCH" in
    arm) ARCH=armv7hf ;;
esac

# ARCH_SHORT ниже соответствует имени CI-артефакта (ARCHITECTURE.md §3.4):
# Linux — полные имена arch (x86_64, i686, arm, arm64); Linaro-кросс получает
# суффикс '-linaro'. Deployer выводит это из env CONAN_USER_TOOLCHAIN.
case "$ARCH" in
    x86_64)
        DEFAULT_BASE_TAG="latest"
        BASE_IMAGE_NAME="gcc84-build-x86_64"
        PROFILE_REL="profiles/lin-gcc84-x86_64"
        TC_PATH=""                  # нативная — без user_toolchain
        ARCH_SHORT="x86_64"         # CI: googletest.lin.gcc84.shared.x86_64.<ver>.nupkg
        IMAGE_TAG="grpc-tc-mirror-x86_64"
        ;;
    i686)
        DEFAULT_BASE_TAG="latest"
        BASE_IMAGE_NAME="gcc84-build-x86_64"   # multilib — i686 живёт в x86_64-образе
        PROFILE_REL="profiles/lin-gcc84-i686"
        TC_PATH=""
        ARCH_SHORT="i686"           # CI: googletest.lin.gcc84.shared.i686.<ver>.nupkg
        IMAGE_TAG="grpc-tc-mirror-i686"
        ;;
    armv7hf)
        DEFAULT_BASE_TAG="0.1.0"
        BASE_IMAGE_NAME="gcc75-build-arm"
        PROFILE_REL="profiles/lin-gcc75-arm-linaro"
        TC_PATH="/work/conan-recipes/profiles/toolchains/linaro-arm.cmake"
        ARCH_SHORT="arm-linaro"     # CI: googletest.lin.gcc75.shared.arm-linaro.<ver>.nupkg
        IMAGE_TAG="grpc-tc-mirror-armv7hf"
        ;;
    arm64)
        DEFAULT_BASE_TAG="0.1.0"
        BASE_IMAGE_NAME="gcc75-build-arm64"
        PROFILE_REL="profiles/lin-gcc-aarch64-linaro"
        TC_PATH="/work/conan-recipes/profiles/toolchains/linaro-aarch64.cmake"
        ARCH_SHORT="arm64-linaro"   # CI: googletest.lin.gcc75.shared.arm64-linaro.<ver>.nupkg
        IMAGE_TAG="grpc-tc-mirror-arm64"
        ;;
    *)
        echo "[FAIL] arch must be x86_64|i686|armv7hf|arm|arm64, got '$ARCH'" >&2
        exit 2
        ;;
esac

BASE_IMAGE_TAG="${BASE_IMAGE_TAG:-$DEFAULT_BASE_TAG}"

# Резолв BASE_IMAGE — LOCAL_BASE перебивает путь в registry.
if [[ -n "${LOCAL_BASE:-}" ]]; then
    BASE_IMAGE="$LOCAL_BASE"
    IMAGE_TAG="${IMAGE_TAG}-local"
else
    : "${REGISTRY:?env REGISTRY must be set (or use LOCAL_BASE for mac/dev mode)}"
    BASE_IMAGE="$REGISTRY/library/${BASE_IMAGE_NAME}:${BASE_IMAGE_TAG}"
fi

PROFILE_HOST="/work/conan-recipes/${PROFILE_REL}"
PROFILE_BUILD="/work/conan-recipes/profiles/lin-gcc84-x86_64"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
OUTPUT_DIR="$ROOT_DIR/output-${ARCH}"
CACHE_DIR="$OUTPUT_DIR/conan-cache"

# Флаг QEMU-платформы — только когда arch хоста отличается от целевой.
# Mac M-series + LOCAL_BASE = пользователь хочет эмуляцию образа linux/amd64.
PLATFORM_FLAG=""
if [[ -n "${LOCAL_BASE:-}" ]] && [[ "$(uname -m)" == "arm64" ]]; then
    PLATFORM_FLAG="--platform linux/amd64"
fi

PASS=0; FAIL=0
pass() { echo "[PASS] $*"; PASS=$((PASS + 1)); }
fail() { echo "[FAIL] $*" >&2; FAIL=$((FAIL + 1)); }
hdr()  { echo ""; echo "== $* =="; }

hdr "Pre-flight"
echo "[INFO] arch:       $ARCH (short=$ARCH_SHORT)"
echo "[INFO] base image: $BASE_IMAGE"
echo "[INFO] profile h:  $PROFILE_REL"
echo "[INFO] profile b:  profiles/lin-gcc84-x86_64"
echo "[INFO] toolchain:  ${TC_PATH:-<none — native>}"
echo "[INFO] image tag:  $IMAGE_TAG"
echo "[INFO] output:     $OUTPUT_DIR"
echo "[INFO] platform:   ${PLATFORM_FLAG:-<host native>}"

if ! command -v docker >/dev/null; then
    fail "docker not on PATH"; exit 2
fi
# Автоопределение, нужен ли docker'у sudo (Astra: да, Mac: обычно нет).
if docker info >/dev/null 2>&1; then
    DOCKER="docker"
elif sudo -n docker info >/dev/null 2>&1; then
    DOCKER="sudo docker"
else
    fail "docker daemon not reachable as user nor via passwordless sudo"
    fail "  → on Astra: run with sudo, or configure passwordless sudo for docker"
    fail "  → on Mac: ensure Docker Desktop is running and user can call docker"
    exit 2
fi
pass "docker daemon reachable (via: $DOCKER)"

# Pull базового образа (тихо пропускаем, если LOCAL_BASE — локальный образ).
hdr "1. Base image $BASE_IMAGE"
if [[ -n "${LOCAL_BASE:-}" ]]; then
    if $DOCKER image inspect "$BASE_IMAGE" >/dev/null 2>&1; then
        pass "local base image present"
    else
        fail "LOCAL_BASE='$LOCAL_BASE' not found locally"; exit 1
    fi
else
    if $DOCKER pull $PLATFORM_FLAG "$BASE_IMAGE" >/tmp/pull-$ARCH.log 2>&1; then
        pass "pulled"
    else
        fail "pull failed; see /tmp/pull-$ARCH.log"
        tail -10 /tmp/pull-$ARCH.log
        exit 1
    fi
fi

# Сборка образа grpc-tc-mirror поверх базового. По одному Dockerfile на build-arch;
# i686 переиспользует x86_64-файл (та же gcc84-база, профиль задаётся через -e PROFILE
# в рантайме), armv7hf использует arm-файл.
case "$ARCH" in
    x86_64|i686) DOCKERFILE_ARCH=x86_64 ;;
    armv7hf)     DOCKERFILE_ARCH=arm ;;
    arm64)       DOCKERFILE_ARCH=arm64 ;;
    *)           DOCKERFILE_ARCH="$ARCH" ;;
esac
hdr "2. docker build $IMAGE_TAG"
cd "$ROOT_DIR"
if $DOCKER build $PLATFORM_FLAG \
        --build-arg BASE_IMAGE="$BASE_IMAGE" \
        -f "Dockerfile.grpc-tc-mirror-$DOCKERFILE_ARCH" \
        -t "$IMAGE_TAG" \
        . >/tmp/build-$ARCH.log 2>&1; then
    pass "built $IMAGE_TAG"
else
    fail "build failed; tail of /tmp/build-$ARCH.log:"
    tail -20 /tmp/build-$ARCH.log
    exit 1
fi

if [[ "$MODE" == "smoke" ]]; then
    echo ""
    echo "================ SMOKE SUMMARY ================"
    echo " arch=$ARCH  pass=$PASS  fail=$FAIL"
    echo "==============================================="
    [[ "$FAIL" -eq 0 ]] || exit 1
    echo "Smoke OK. To run the full build:"
    echo "  $0 build $ARCH"
    exit 0
fi

# Полная сборка.
hdr "3. Full Conan build (15-90 min depending on arch + emulation)"
mkdir -p "$OUTPUT_DIR" "$CACHE_DIR"

if [[ -n "${FORCE_REBUILD:-}" ]]; then
    echo "[INFO] FORCE_REBUILD set — wiping cache + nupkg"
    sudo rm -rf "$CACHE_DIR"/* "$OUTPUT_DIR"/*.nupkg 2>/dev/null
    mkdir -p "$CACHE_DIR"
fi

DOCKER_ARGS=(
    --rm $PLATFORM_FLAG
    --memory=6g --memory-swap=10g
    -v "$ROOT_DIR:/work/conan-recipes"
    -v "$CACHE_DIR:/root/.conan2"
    -v "$OUTPUT_DIR:/work/conan-recipes/output"
    -v "$OUTPUT_DIR:/host"
    -e PROFILE="$PROFILE_HOST"
    -e PROFILE_BUILD="$PROFILE_BUILD"
    -e SHARED="${SHARED:-False}"
    -e CMAKE_BUILD_PARALLEL_LEVEL="${CMAKE_BUILD_PARALLEL_LEVEL:-4}"
)

# CONAN_USER_TOOLCHAIN передаём только для кросс-архитектур. Для нативных
# x86_64/i686 пусто — env-fallback в рецептах огорожен по arch (коммит 8278d55),
# так что для нативной это всё равно no-op, но делаем явно.
if [[ -n "$TC_PATH" ]]; then
    DOCKER_ARGS+=(-e "CONAN_USER_TOOLCHAIN=$TC_PATH")
fi

if $DOCKER run "${DOCKER_ARGS[@]}" "$IMAGE_TAG" bash -c '
        ./test-astra/run_test_grpc.sh 2>&1 | tee /host/run.log
        rc=$?
        echo ""
        echo "=== EXIT $rc ==="
        echo "=== LINARO-ARM-TC count ==="
        grep -c "==LINARO-ARM-TC==" /host/run.log
        echo "=== LINARO-ARM64-TC count ==="
        grep -c "==LINARO-ARM64-TC==" /host/run.log
        exit $rc
    '; then
    pass "container exited 0"
else
    fail "container exited non-zero"
    echo "[INFO] tail of run.log:"
    tail -30 "$OUTPUT_DIR/run.log"
fi

# Проверка .nupkg-артефактов.
hdr "4. Verify $OUTPUT_DIR/*.nupkg"
expected_count=7
actual_count=$(ls -1 "$OUTPUT_DIR"/*.nupkg 2>/dev/null | wc -l)
if [[ "$actual_count" -eq "$expected_count" ]]; then
    pass "$actual_count .nupkg files"
else
    fail "expected $expected_count files, got $actual_count"
fi

# Каждое имя должно содержать '.lin.gcc.<linkage>.<ARCH_SHORT>.' — default-слот
# `shared` (GR113/DynamicRT, см. INFRASTRUCTURE.md §3.7). Brace-expansion ловит и
# static (GR121-слот) для LEGACY_NUPKG_LINKAGE=static.
for pkg in grpc protobuf abseil openssl re2 c-ares zlib; do
    f=$(ls -1 "$OUTPUT_DIR/$pkg".lin.gcc.{static,shared}."$ARCH_SHORT".*.nupkg 2>/dev/null | head -1)
    if [[ -n "$f" ]]; then
        size=$(du -h "$f" | cut -f1)
        pass "$(basename "$f") ($size)"
    else
        fail "$pkg.lin.gcc.{static,shared}.$ARCH_SHORT.*.nupkg missing"
    fi
done

echo ""
echo "================ BUILD SUMMARY ================"
echo " arch=$ARCH  pass=$PASS  fail=$FAIL"
echo " output: $OUTPUT_DIR/"
ls -la "$OUTPUT_DIR"/*.nupkg 2>/dev/null | awk '{print "   " $NF " (" $5 " bytes)"}'
echo "==============================================="

[[ "$FAIL" -eq 0 ]]
