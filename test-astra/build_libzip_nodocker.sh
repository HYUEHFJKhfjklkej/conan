#!/bin/bash
# build_libzip_nodocker.sh — собрать пакет libzip/1.11.4 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> libzip.<...>.nupkg. Никакого
...
# libzip — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_libzip_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_libzip_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_libzip_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LIBZIP_VERSION   версия libzip (по умолч. 1.11.4)
#   PKG_VERSION   generic-фолбэк версии (если LIBZIP_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-libzip-<arch>)
...
LIBZIP_VERSION="${LIBZIP_VERSION:-${PKG_VERSION:-1.11.4}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-libzip-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт libzip читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта libzip.
conan export zlib/ --version=1.3.1 --no-remote   # dep
conan export libzip/ --version="$LIBZIP_VERSION" --no-remote
