#!/bin/bash
# build_sqlite3_nodocker.sh — собрать пакет sqlite3/3.53.3 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> sqlite3.<...>.nupkg. Никакого
...
# sqlite3 — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_sqlite3_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_sqlite3_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_sqlite3_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   SQLITE3_VERSION   версия sqlite3 (по умолч. 3.53.3)
#   PKG_VERSION   generic-фолбэк версии (если SQLITE3_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-sqlite3-<arch>)
...
SQLITE3_VERSION="${SQLITE3_VERSION:-${PKG_VERSION:-3.53.3}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-sqlite3-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт sqlite3 читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта sqlite3.
conan export sqlite3/ --version="$SQLITE3_VERSION" --no-remote
