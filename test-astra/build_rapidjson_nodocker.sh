#!/bin/bash
# build_rapidjson_nodocker.sh — собрать пакет rapidjson/cci.20250205 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> rapidjson.<...>.nupkg. Никакого
...
# rapidjson — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_rapidjson_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_rapidjson_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_rapidjson_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   RAPIDJSON_VERSION   версия rapidjson (по умолч. cci.20250205)
#   PKG_VERSION   generic-фолбэк версии (если RAPIDJSON_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-rapidjson-<arch>)
...
RAPIDJSON_VERSION="${RAPIDJSON_VERSION:-${PKG_VERSION:-cci.20250205}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-rapidjson-$ARCH}"
SHARED="${SHARED:-False}"
...
# rapidjson header-only: компиляции нет, тулчейн ни на что не влияет. Переменная
# выставляется только ради единообразия с остальными драйверами.
...
REF="rapidjson/$RAPIDJSON_VERSION"
mkdir -p "$OUTPUT_DIR"
...
# Экспорт рецепта rapidjson.
conan export rapidjson/ --version="$RAPIDJSON_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/rapidjson.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
