#!/bin/bash
# build_libssh2_nodocker.sh — собрать пакет libssh2/1.11.1 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> libssh2.<...>.nupkg. Никакого
...
# libssh2 — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_libssh2_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_libssh2_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_libssh2_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LIBSSH2_VERSION   версия libssh2 (по умолч. 1.11.1)
#   PKG_VERSION   generic-фолбэк версии (если LIBSSH2_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-libssh2-<arch>)
...
LIBSSH2_VERSION="${LIBSSH2_VERSION:-${PKG_VERSION:-1.11.1}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-libssh2-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт libssh2 читает CONAN_USER_TOOLCHAIN
