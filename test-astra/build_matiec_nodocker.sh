#!/bin/bash
# build_matiec_nodocker.sh — собрать пакет matiec/0.1.20260512 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> matiec.<...>.nupkg. Никакого
...
# matiec — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_matiec_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_matiec_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_matiec_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   MATIEC_VERSION   версия matiec (по умолч. 0.1.20260512)
#   PKG_VERSION   generic-фолбэк версии (если MATIEC_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-matiec-<arch>)
...
MATIEC_VERSION="${MATIEC_VERSION:-${PKG_VERSION:-0.1.20260512}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-matiec-$ARCH}"
SHARED="${SHARED:-False}"
...
# в имени .nupkg + выбор target-binutils. NB: кросс-собранный iec2c
# запускается на ЦЕЛЕВОЙ арке (это инструмент), не на станке.
...
# Экспорт рецепта matiec.
conan export matiec/ --version="$MATIEC_VERSION" --no-remote
