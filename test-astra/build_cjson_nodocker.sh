#!/bin/bash
# build_cjson_nodocker.sh — собрать пакет cjson/1.7.19 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> cjson.<...>.nupkg. Никакого
...
# cjson — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_cjson_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_cjson_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_cjson_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   CJSON_VERSION   версия cjson (по умолч. 1.7.19)
#   PKG_VERSION   generic-фолбэк версии (если CJSON_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-cjson-<arch>)
...
CJSON_VERSION="${CJSON_VERSION:-${PKG_VERSION:-1.7.19}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-cjson-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт cjson читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта cjson.
conan export cjson/ --version="$CJSON_VERSION" --no-remote
