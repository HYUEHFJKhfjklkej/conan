#!/bin/bash
# build_dbus_nodocker.sh — собрать пакет dbus/1.15.8 БЕЗ docker-self-wrap.
#
# Единственный meson-пакет конвейера: драйвер сам доставляет meson/ninja
# офлайн из packages-linux/, если их нет в образе (см. блок ниже).
#
# export -> build Release+Debug -> deployer -> dbus.<...>.nupkg. Никакого
...
# dbus — одиночный пакет вне grpc-дерева (dep: expat).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_dbus_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_dbus_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_dbus_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   DBUS_VERSION   версия dbus (по умолч. 1.15.8)
#   PKG_VERSION   generic-фолбэк версии (если DBUS_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-dbus-<arch>)
...
DBUS_VERSION="${DBUS_VERSION:-${PKG_VERSION:-1.15.8}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-dbus-$ARCH}"
SHARED="${SHARED:-False}"
...
# Кросс-компилятор meson берёт из [buildenv] host-профиля: MesonToolchain пишет
# CC/CXX/AR/STRIP в [binaries] cross-файла; CMake-депу expat хватает тех же
# CC/CXX из env. CONAN_USER_TOOLCHAIN нужен только deployer'у — суффикс арки
# `-linaro` в имени .nupkg + выбор target-binutils.
...
# dbus собирается Meson'ом: в старых образах станка meson/ninja нет — доставим
# офлайн из packages-linux/ (pure-py + manylinux колёса; в свежих образах вшиты).
if ! command -v meson >/dev/null 2>&1 || ! command -v ninja >/dev/null 2>&1; then
    PYBIN="$(command -v python3 || true)"
    [ -x /opt/python/bin/python3 ] && PYBIN=/opt/python/bin/python3
    [ -n "$PYBIN" ] || { echo "[FAIL] python3 не найден (нужен для доставки meson/ninja)" >&2; exit 1; }
    "$PYBIN" -m pip install --no-index --find-links=packages-linux meson ninja
    PYDIR="$(dirname "$PYBIN")"
    for t in meson ninja; do
        command -v "$t" >/dev/null 2>&1 || ln -sf "$PYDIR/$t" /usr/local/bin/"$t" 2>/dev/null || true
    done
fi
command -v meson >/dev/null 2>&1 || { echo "[FAIL] meson недоступен (нет в PATH и не встал из packages-linux)" >&2; exit 1; }
command -v ninja >/dev/null 2>&1 || { echo "[FAIL] ninja недоступен (нет в PATH и не встал из packages-linux)" >&2; exit 1; }

REF="dbus/$DBUS_VERSION"
mkdir -p "$OUTPUT_DIR"
...
# Экспорт рецепта dbus.
conan export expat/ --version=2.8.2 --no-remote   # dep
conan export dbus/ --version="$DBUS_VERSION" --no-remote
...
# Deployer эмитит .nupkg на КАЖДЫЙ host-dep графа — чистим и expat, иначе
# stale-артефакт от прошлого прогона уедет в publish.
rm -f "$OUTPUT_DIR"/{dbus,expat}.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
