#!/bin/bash
# build_qwindowkit_nodocker.sh — собрать пакет qwindowkit/1.5.0 БЕЗ docker-self-wrap.
#
...
# Qt не несут. Потребители — десктопные Qt GUI (какие именно — не вскрыто).
...
#
# Версия 1.5.0 = upstream latest (легаси-пина нет: QWINDOWKIT на фото не вскрыт).
#
# export -> build Release+Debug -> deployer -> qwindowkit.<...>.nupkg.
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ./test-astra/build_qwindowkit_nodocker.sh              # x86_64 (единственный)
#
# Env:
#   QWINDOWKIT_VERSION   версия qwindowkit (по умолч. 1.5.0)
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-qwindowkit-x86_64)
...
QWINDOWKIT_VERSION="${QWINDOWKIT_VERSION:-${PKG_VERSION:-1.5.0}}"
ARCH="${ARCH:-x86_64}"
if [ "$ARCH" != "x86_64" ]; then
    echo "[FAIL] qwindowkit собирается только на x86_64 (Qt 5.15.2 есть лишь в образе gcc84-build-x86_64; arm/arm64-образы Qt не несут)" >&2
    exit 2
fi
PROFILE="${PROFILE:-profiles/lin-gcc84-x86_64}"
PROFILE_BUILD="${PROFILE_BUILD:-$PROFILE}"
OUTPUT_DIR="${OUTPUT_DIR:-output-qwindowkit-x86_64}"
SHARED="${SHARED:-False}"
...
REF="qwindowkit/$QWINDOWKIT_VERSION"
mkdir -p "$OUTPUT_DIR"
...
conan export qwindowkit/ --version="$QWINDOWKIT_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/qwindowkit.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
