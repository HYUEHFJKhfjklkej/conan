#!/bin/bash
# build_jansson_nodocker.sh — собрать пакет jansson/2.15.1 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> jansson.<...>.nupkg. Никакого
...
# jansson — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_jansson_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_jansson_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_jansson_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   JANSSON_VERSION   версия jansson (по умолч. 2.15.1)
#   PKG_VERSION   generic-фолбэк версии (если JANSSON_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-jansson-<arch>)
...
JANSSON_VERSION="${JANSSON_VERSION:-${PKG_VERSION:-2.15.1}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-jansson-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт jansson читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта jansson.
conan export jansson/ --version="$JANSSON_VERSION" --no-remote
