#!/bin/bash
# build_snap7_nodocker.sh — собрать пакет snap7/1.4.2 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> snap7.<...>.nupkg. Никакого
...
# snap7 — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_snap7_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_snap7_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_snap7_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   SNAP7_VERSION   версия snap7 (по умолч. 1.4.2)
#   PKG_VERSION   generic-фолбэк версии (если SNAP7_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-snap7-<arch>)
...
SNAP7_VERSION="${SNAP7_VERSION:-${PKG_VERSION:-1.4.2}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-snap7-$ARCH}"
SHARED="${SHARED:-False}"
...
# Экспорт рецепта snap7.
conan export snap7/ --version="$SNAP7_VERSION" --no-remote
