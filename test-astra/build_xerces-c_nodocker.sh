#!/bin/bash
# build_xerces-c_nodocker.sh — собрать пакет xerces-c/3.3.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> xerces-c.<...>.nupkg. Никакого
...
# xerces-c — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_xerces-c_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_xerces-c_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_xerces-c_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   XERCES_C_VERSION   версия xerces-c (по умолч. 3.3.0)
#   PKG_VERSION   generic-фолбэк версии (если XERCES_C_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-xerces-c-<arch>)
...
XERCES_C_VERSION="${XERCES_C_VERSION:-${PKG_VERSION:-3.3.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-xerces-c-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт xerces-c читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта xerces-c.
conan export xerces-c/ --version="$XERCES_C_VERSION" --no-remote
