#!/bin/bash
# build_nanopb_nodocker.sh — собрать пакет nanopb/0.4.9.1 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> nanopb.<...>.nupkg. Никакого
...
# nanopb — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_nanopb_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_nanopb_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_nanopb_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   NANOPB_VERSION   версия nanopb (по умолч. 0.4.9.1)
#   PKG_VERSION   generic-фолбэк версии (если NANOPB_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-nanopb-<arch>)
...
NANOPB_VERSION="${NANOPB_VERSION:-${PKG_VERSION:-0.4.9.1}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-nanopb-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт nanopb читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта nanopb.
conan export nanopb/ --version="$NANOPB_VERSION" --no-remote
