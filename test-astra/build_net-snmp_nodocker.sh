#!/bin/bash
# build_net-snmp_nodocker.sh — собрать пакет net-snmp/5.9.4 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> net-snmp.<...>.nupkg. Никакого
...
# net-snmp — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_net-snmp_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_net-snmp_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_net-snmp_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   NET_SNMP_VERSION   версия net-snmp (по умолч. 5.9.4)
#   PKG_VERSION   generic-фолбэк версии (если NET_SNMP_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-net-snmp-<arch>)
...
NET_SNMP_VERSION="${NET_SNMP_VERSION:-${PKG_VERSION:-5.9.4}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-net-snmp-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт net-snmp читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта net-snmp.
conan export zlib/ --version=1.3.1 --no-remote   # dep
conan export openssl/ --version=3.4.5 --no-remote   # dep
conan export pcre/ --version=8.45 --no-remote   # dep
conan export net-snmp/ --version="$NET_SNMP_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/net-snmp.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "pcre/*:build_pcregrep=False" \
