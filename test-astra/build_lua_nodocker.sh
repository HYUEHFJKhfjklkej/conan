#!/bin/bash
# build_lua_nodocker.sh — собрать пакет lua/5.5.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> lua.<...>.nupkg. Никакого
...
# lua — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_lua_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_lua_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_lua_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LUA_VERSION   версия lua (по умолч. 5.5.0)
#   PKG_VERSION   generic-фолбэк версии (если LUA_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-lua-<arch>)
...
LUA_VERSION="${LUA_VERSION:-${PKG_VERSION:-5.5.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-lua-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт lua читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта lua.
conan export lua/ --version="$LUA_VERSION" --no-remote
