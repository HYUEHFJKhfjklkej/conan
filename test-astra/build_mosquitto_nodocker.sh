#!/bin/bash
# build_mosquitto_nodocker.sh — собрать пакет mosquitto/2.0.22 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> mosquitto.<...>.nupkg. Никакого
...
# mosquitto — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_mosquitto_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_mosquitto_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_mosquitto_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   MOSQUITTO_VERSION   версия mosquitto (по умолч. 2.0.22)
#   PKG_VERSION   generic-фолбэк версии (если MOSQUITTO_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-mosquitto-<arch>)
...
MOSQUITTO_VERSION="${MOSQUITTO_VERSION:-${PKG_VERSION:-2.0.22}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-mosquitto-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт mosquitto читает CONAN_USER_TOOLCHAIN
...
REF="mosquitto/$MOSQUITTO_VERSION"
mkdir -p "$OUTPUT_DIR"
...
rm -f "$OUTPUT_DIR"/mosquitto.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "mosquitto/*:cjson=False" \
