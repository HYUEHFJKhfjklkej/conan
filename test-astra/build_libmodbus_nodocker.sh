#!/bin/bash
# build_libmodbus_nodocker.sh — собрать пакет libmodbus/3.1.12 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> libmodbus.<...>.nupkg. Никакого
...
# libmodbus — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_libmodbus_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_libmodbus_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_libmodbus_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LIBMODBUS_VERSION   версия libmodbus (по умолч. 3.1.12)
#   PKG_VERSION   generic-фолбэк версии (если LIBMODBUS_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-libmodbus-<arch>)
...
LIBMODBUS_VERSION="${LIBMODBUS_VERSION:-${PKG_VERSION:-3.1.12}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-libmodbus-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт libmodbus читает CONAN_USER_TOOLCHAIN
...
REF="libmodbus/$LIBMODBUS_VERSION"
mkdir -p "$OUTPUT_DIR"
...
# Экспорт рецепта libmodbus.
conan export libmodbus/ --version="$LIBMODBUS_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/libmodbus.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
