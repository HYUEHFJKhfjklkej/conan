#!/bin/bash
# build_nlohmann_json_nodocker.sh — собрать пакет nlohmann_json/3.12.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> nlohmann_json.<...>.nupkg. Никакого
...
# nlohmann_json — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_nlohmann_json_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_nlohmann_json_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_nlohmann_json_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   NLOHMANN_JSON_VERSION   версия nlohmann_json (по умолч. 3.12.0)
#   PKG_VERSION   generic-фолбэк версии (если NLOHMANN_JSON_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-nlohmann_json-<arch>)
...
NLOHMANN_JSON_VERSION="${NLOHMANN_JSON_VERSION:-${PKG_VERSION:-3.12.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-nlohmann_json-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт nlohmann_json читает CONAN_USER_TOOLCHAIN
...
REF="nlohmann_json/$NLOHMANN_JSON_VERSION"
mkdir -p "$OUTPUT_DIR"
...
# Экспорт рецепта nlohmann_json.
conan export nlohmann_json/ --version="$NLOHMANN_JSON_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/nlohmann_json.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
