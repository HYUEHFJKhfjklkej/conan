#!/bin/bash
# build_tinyxml2_nodocker.sh — собрать пакет tinyxml2/11.0.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> tinyxml2.<...>.nupkg. Никакого
...
# tinyxml2 — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_tinyxml2_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_tinyxml2_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_tinyxml2_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   TINYXML2_VERSION   версия tinyxml2 (по умолч. 11.0.0)
#   PKG_VERSION   generic-фолбэк версии (если TINYXML2_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-tinyxml2-<arch>)
...
TINYXML2_VERSION="${TINYXML2_VERSION:-${PKG_VERSION:-11.0.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-tinyxml2-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт tinyxml2 читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта tinyxml2.
conan export tinyxml2/ --version="$TINYXML2_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/tinyxml2.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
