#!/bin/bash
# build_zeromq_nodocker.sh — собрать пакет zeromq/4.3.5 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> zeromq.<...>.nupkg. Никакого
...
# zeromq — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_zeromq_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_zeromq_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_zeromq_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   ZEROMQ_VERSION   версия zeromq (по умолч. 4.3.5)
#   PKG_VERSION   generic-фолбэк версии (если ZEROMQ_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-zeromq-<arch>)
...
ZEROMQ_VERSION="${ZEROMQ_VERSION:-${PKG_VERSION:-4.3.5}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-zeromq-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт zeromq читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта zeromq.
conan export zeromq/ --version="$ZEROMQ_VERSION" --no-remote
