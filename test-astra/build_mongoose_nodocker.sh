#!/bin/bash
# build_mongoose_nodocker.sh — собрать пакет mongoose/7.22 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> mongoose.<...>.nupkg. Никакого
...
# mongoose — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_mongoose_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_mongoose_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_mongoose_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   MONGOOSE_VERSION   версия mongoose (по умолч. 7.22)
#   PKG_VERSION   generic-фолбэк версии (если MONGOOSE_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-mongoose-<arch>)
...
MONGOOSE_VERSION="${MONGOOSE_VERSION:-${PKG_VERSION:-7.22}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-mongoose-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт mongoose читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта mongoose.
conan export mongoose/ --version="$MONGOOSE_VERSION" --no-remote
...
rm -f "$OUTPUT_DIR"/mongoose.*.nupkg
conan install --requires="$REF" \
    -pr:h="$PROFILE" -pr:b="$PROFILE_BUILD" \
    "${REMOTE_ARGS[@]}" \
    -o "*/*:shared=$SHARED" \
