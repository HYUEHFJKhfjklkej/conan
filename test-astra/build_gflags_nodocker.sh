#!/bin/bash
# build_gflags_nodocker.sh — собрать пакет gflags/2.3.0 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> gflags.<...>.nupkg. Никакого
...
# gflags — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_gflags_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_gflags_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_gflags_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   GFLAGS_VERSION   версия gflags (по умолч. 2.3.0)
#   PKG_VERSION   generic-фолбэк версии (если GFLAGS_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-gflags-<arch>)
...
GFLAGS_VERSION="${GFLAGS_VERSION:-${PKG_VERSION:-2.3.0}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-gflags-$ARCH}"
SHARED="${SHARED:-False}"
...
# Экспорт рецепта gflags.
conan export gflags/ --version="$GFLAGS_VERSION" --no-remote
