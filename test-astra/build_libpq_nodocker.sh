#!/bin/bash
# build_libpq_nodocker.sh — собрать пакет libpq/16.14 БЕЗ docker-self-wrap.
#
# export -> build Release+Debug -> deployer -> libpq.<...>.nupkg. Никакого
...
# libpq — одиночный пакет вне grpc-дерева (без транзитивных deps).
#
# Запуск (TC build-step, внутри docker-контейнера от TC):
#   ARCH=x86_64 ./test-astra/build_libpq_nodocker.sh      # x86_64 (по умолч.)
#   ARCH=arm    ./test-astra/build_libpq_nodocker.sh      # кросс armv7hf
#   ARCH=arm64  ./test-astra/build_libpq_nodocker.sh      # кросс arm64
#
# Env:
#   ARCH          x86_64|x86|arm|arm64 (по умолч. x86_64) — выбирает профиль/тулчейн
#   LIBPQ_VERSION   версия libpq (по умолч. 16.14)
#   PKG_VERSION   generic-фолбэк версии (если LIBPQ_VERSION не задан) — единая
...
#   OUTPUT_DIR    выход отн. репо (по умолч. output-libpq-<arch>)
...
LIBPQ_VERSION="${LIBPQ_VERSION:-${PKG_VERSION:-16.14}}"
ARCH="${ARCH:-x86_64}"
case "$ARCH" in
...
OUTPUT_DIR="${OUTPUT_DIR:-output-libpq-$ARCH}"
SHARED="${SHARED:-False}"
...
# Self-wrap'а тут нет, экспортируем сами. Рецепт libpq читает CONAN_USER_TOOLCHAIN
...
# Экспорт рецепта libpq.
conan export libpq/ --version="$LIBPQ_VERSION" --no-remote
