...
LEGACY_DEP_VERSION_MAP = {
    ...
    "abseil": "0.2.0",
    # У rapidjson апстрим-релиз один и древний (1.1.0, 2016); все берут master,
    # CCI версионирует снимки как `cci.<дата>`. NuGet такую строку не принимает
    # (нужен числовой semver), поэтому снимок cci.20250205 объявляется как
    # 1.1.20250205: мажор/минор от последнего релиза, patch = дата снимка.
    "rapidjson": "1.1.20250205",
}
