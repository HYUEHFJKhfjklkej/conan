...
# 3. protobuf libprotolib.so симлинк на libprotoc.so
unzip -l <repo>/output-grpc-1601-upstream/protobuf.lin.gcc84.shared.x86_64.4.25.2.1.nupkg | grep -E 'libprotolib|libprotoc'
# Жду libprotoc.so, libprotoc.so.25.2.0, libprotolib.so, libprotolib.so.25.2.0
# (последние два — симлинки на первые)

# 4. zlib libzlib.so симлинк на libz.so
unzip -l <repo>/output-grpc-1601-upstream/zlib.lin.gcc84.shared.x86_64.1.3.0.1.nupkg | grep -E 'libz|libzlib'
# Жду libz.so, libz.so.1, libz.so.1.3.0, libzlib.so → симлинк
```
...
RUNPATH не propagates транзитивно у Linux loader'а. На runtime у потребителя:
```bash
