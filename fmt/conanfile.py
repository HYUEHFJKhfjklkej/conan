...
class FmtConan(ConanFile):
    ...
    def export_sources(self):
        ...
        # fmt публикует релизы .zip (не .tar.gz), поэтому пакуем src/*.zip.
        copy(self, "src/*.zip", src=self.recipe_folder, dst=self.export_sources_folder)
    ...
    def generate(self):
        if not self.options.header_only:
            ...
            # ARM cross: *:user_toolchain из [conf] не доходит до транзитивных
            # deps, поэтому берём его из env и вкладываем в user_toolchain-блок
            # (тот же env-fallback, что в abseil/re2/protobuf/grpc). Арк-гейт
            # обязателен — иначе тулчейн течёт в build-контекст (x86_64).
