...
class NetSnmpConan(ConanFile):
    ...
    def build_requirements(self):
        if is_msvc(self):
            ...
            # Offline-патч: gnu-config убран (его source() клонирует git.savannah,
            # оффлайн недоступно). net-snmp несёт свой config.sub/config.guess,
            # покрывающий наши арки (x86_64/i686/arm/aarch64); обновление не нужно.
            self.tool_requires("autoconf/2.71")
            self.tool_requires("automake/1.16.5")
            self.tool_requires("libtool/2.4.7")
            # только autotools-путь: msvc собирается NMake'ом без pkg-config,
            # а pkgconf офлайн нерезолвим (нет в win platform_tool_requires)
