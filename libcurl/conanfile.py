...
class LibcurlConan(ConanFile):
    ...
    def source(self):
        ...
        # Offline-патч: CA-бандл берём из export_sources (src/cacert.pem). При
        # layout src_folder="src" он уже оказывается в source_folder — тогда
        # ничего не качаем; иначе — сетевой download (как в оригинале).
        if not os.path.isfile(os.path.join(self.source_folder, "cacert.pem")):
