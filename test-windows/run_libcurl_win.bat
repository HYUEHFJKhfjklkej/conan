@echo off
:: ============================================================
::  Сборка пакета libcurl/8.21.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_libcurl_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. libcurl — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> libcurl.win.*.nupkg
::  в output-libcurl-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_libcurl_win.bat
::  Env:
::    LIBCURL_VERSION       по умолчанию 8.21.0
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-libcurl-win
...
if "%LIBCURL_VERSION%"=="" set LIBCURL_VERSION=8.21.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-libcurl-win
...
echo [INFO] Ref:      libcurl/%LIBCURL_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export libcurl recipe
echo ============================================
conan export "%ROOT_DIR%\zlib" --version=1.3.1 --no-remote
if errorlevel 1 exit /b 1
conan export "%ROOT_DIR%\openssl" --version=3.4.5 --no-remote
if errorlevel 1 exit /b 1
echo [INFO] conan export libcurl (%LIBCURL_VERSION%)
conan export "%ROOT_DIR%\libcurl" --version=%LIBCURL_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] libcurl export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build libcurl/%LIBCURL_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building libcurl/%LIBCURL_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=libcurl/%LIBCURL_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] libcurl %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] libcurl/%LIBCURL_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\libcurl.win.*.nupkg" (
    echo [INFO] libcurl.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] libcurl.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] libcurl %LIBCURL_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
