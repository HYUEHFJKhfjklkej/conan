@echo off
:: ============================================================
::  Сборка пакета xerces-c/3.3.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_xerces-c_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. xerces-c — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> xerces-c.win.*.nupkg
::  в output-xerces-c-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_xerces-c_win.bat
::  Env:
::    XERCES_C_VERSION       по умолчанию 3.3.0
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-xerces-c-win
...
if "%XERCES_C_VERSION%"=="" set XERCES_C_VERSION=3.3.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-xerces-c-win
...
echo [INFO] Ref:      xerces-c/%XERCES_C_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export xerces-c recipe
echo ============================================
echo [INFO] conan export xerces-c (%XERCES_C_VERSION%)
conan export "%ROOT_DIR%\xerces-c" --version=%XERCES_C_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] xerces-c export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build xerces-c/%XERCES_C_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building xerces-c/%XERCES_C_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=xerces-c/%XERCES_C_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] xerces-c %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] xerces-c/%XERCES_C_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\xerces-c.win.*.nupkg" (
    echo [INFO] xerces-c.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] xerces-c.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] xerces-c %XERCES_C_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
