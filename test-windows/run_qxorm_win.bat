@echo off
:: ============================================================
::  Сборка пакета qxorm/1.5.1 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_qxorm_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. qxorm — вне grpc-дерева,
...
if "%QXORM_VERSION%"=="" set QXORM_VERSION=1.5.1
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-qxorm-win
...
echo [INFO] Ref:      qxorm/%QXORM_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export qxorm recipe
echo ============================================
echo [INFO] conan export qxorm (%QXORM_VERSION%)
conan export "%ROOT_DIR%\qxorm" --version=%QXORM_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] qxorm export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build qxorm/%QXORM_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building qxorm/%QXORM_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=qxorm/%QXORM_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] qxorm %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] qxorm/%QXORM_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\qxorm.win.*.nupkg" (
    echo [INFO] qxorm.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] qxorm.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] qxorm %QXORM_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
