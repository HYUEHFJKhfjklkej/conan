@echo off
:: ============================================================
::  Сборка пакета nanopb/0.4.9.1 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_nanopb_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. nanopb — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> nanopb.win.*.nupkg
::  в output-nanopb-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_nanopb_win.bat
::  Env:
::    NANOPB_VERSION       по умолчанию 0.4.9.1
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-nanopb-win
...
if "%NANOPB_VERSION%"=="" set NANOPB_VERSION=0.4.9.1
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-nanopb-win
...
echo [INFO] Ref:      nanopb/%NANOPB_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export nanopb recipe
echo ============================================
echo [INFO] conan export nanopb (%NANOPB_VERSION%)
conan export "%ROOT_DIR%\nanopb" --version=%NANOPB_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] nanopb export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build nanopb/%NANOPB_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building nanopb/%NANOPB_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=nanopb/%NANOPB_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] nanopb %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] nanopb/%NANOPB_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\nanopb.win.*.nupkg" (
    echo [INFO] nanopb.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] nanopb.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] nanopb %NANOPB_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
