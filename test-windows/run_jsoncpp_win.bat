@echo off
:: ============================================================
::  Сборка пакета jsoncpp/1.9.8 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_jsoncpp_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. jsoncpp — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> jsoncpp.win.*.nupkg
::  в output-jsoncpp-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_jsoncpp_win.bat
::  Env:
::    JSONCPP_VERSION       по умолчанию 1.9.8
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-jsoncpp-win
...
if "%JSONCPP_VERSION%"=="" set JSONCPP_VERSION=1.9.8
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-jsoncpp-win
...
echo [INFO] Ref:      jsoncpp/%JSONCPP_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export jsoncpp recipe
echo ============================================
echo [INFO] conan export jsoncpp (%JSONCPP_VERSION%)
conan export "%ROOT_DIR%\jsoncpp" --version=%JSONCPP_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] jsoncpp export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build jsoncpp/%JSONCPP_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building jsoncpp/%JSONCPP_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=jsoncpp/%JSONCPP_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] jsoncpp %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] jsoncpp/%JSONCPP_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\jsoncpp.win.*.nupkg" (
    echo [INFO] jsoncpp.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] jsoncpp.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] jsoncpp %JSONCPP_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
