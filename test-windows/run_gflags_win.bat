@echo off
:: ============================================================
::  Сборка пакета gflags/2.3.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_gflags_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. gflags — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> gflags.win.*.nupkg
::  в output-gflags-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_gflags_win.bat
::  Env:
::    GFLAGS_VERSION       по умолчанию 2.3.0
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-gflags-win
...
if "%GFLAGS_VERSION%"=="" set GFLAGS_VERSION=2.3.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-gflags-win
...
echo [INFO] Ref:      gflags/%GFLAGS_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export gflags recipe
echo ============================================
echo [INFO] conan export gflags (%GFLAGS_VERSION%)
conan export "%ROOT_DIR%\gflags" --version=%GFLAGS_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] gflags export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build gflags/%GFLAGS_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building gflags/%GFLAGS_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=gflags/%GFLAGS_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%" -o "gflags/*:nothreads=False"
    if errorlevel 1 (
        echo [FAIL] gflags %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] gflags/%GFLAGS_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\gflags.win.*.nupkg" (
    echo [INFO] gflags.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] gflags.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] gflags %GFLAGS_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
