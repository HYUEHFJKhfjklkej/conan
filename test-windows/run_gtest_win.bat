@echo off
:: ============================================================
::  Сборка пакета gtest/1.17.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_gtest_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. gtest — вне grpc-дерева,
::  без транзитивных deps. Deployer переименовывает gtest -> googletest.
::
::  export -> build Release+Debug -> deployer -> googletest.win.*.nupkg
::  в output-gtest-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_gtest_win.bat
::  Env:
::    GTEST_VERSION     по умолчанию 1.17.0 (последняя upstream)
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-gtest-win
...
if "%GTEST_VERSION%"=="" set GTEST_VERSION=1.17.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-gtest-win
...
echo [INFO] Ref:      gtest/%GTEST_VERSION%  (deployer -> googletest.*.nupkg)
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export gtest recipe
echo ============================================
echo [INFO] conan export gtest (%GTEST_VERSION%)
conan export "%ROOT_DIR%\gtest" --version=%GTEST_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] gtest export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build gtest/%GTEST_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building gtest/%GTEST_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=gtest/%GTEST_VERSION% ^
...
echo [OK] gtest/%GTEST_VERSION% built (Release+Debug)
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 3/3: Package via deployer (gtest -^> googletest)
echo ============================================
...
if exist "%OUTPUT_DIR%\googletest.win.*.nupkg" (
    echo [INFO] googletest.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] googletest.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] googletest %GTEST_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
