@echo off
:: ============================================================
::  Сборка пакета mosquitto/2.0.22 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_mosquitto_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. mosquitto — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> mosquitto.win.*.nupkg
::  в output-mosquitto-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_mosquitto_win.bat
::  Env:
::    MOSQUITTO_VERSION       по умолчанию 2.0.22
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-mosquitto-win
...
if "%MOSQUITTO_VERSION%"=="" set MOSQUITTO_VERSION=2.0.22
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-mosquitto-win
...
echo [INFO] Ref:      mosquitto/%MOSQUITTO_VERSION%
...
conan export "%ROOT_DIR%\mosquitto" --version=%MOSQUITTO_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] mosquitto export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build mosquitto/%MOSQUITTO_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building mosquitto/%MOSQUITTO_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=mosquitto/%MOSQUITTO_VERSION% ^
    -o "mosquitto/*:cjson=False" ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] mosquitto %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] mosquitto/%MOSQUITTO_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\mosquitto.win.*.nupkg" (
    echo [INFO] mosquitto.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] mosquitto.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] mosquitto %MOSQUITTO_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
