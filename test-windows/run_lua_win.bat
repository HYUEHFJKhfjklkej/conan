@echo off
:: ============================================================
::  Сборка пакета lua/5.5.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_lua_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. lua — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> lua.win.*.nupkg
::  в output-lua-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_lua_win.bat
::  Env:
::    LUA_VERSION       по умолчанию 5.5.0
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-lua-win
...
if "%LUA_VERSION%"=="" set LUA_VERSION=5.5.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-lua-win
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export lua recipe
echo ============================================
echo [INFO] conan export lua (%LUA_VERSION%)
conan export "%ROOT_DIR%\lua" --version=%LUA_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] lua export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build lua/%LUA_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building lua/%LUA_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=lua/%LUA_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] lua %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] lua/%LUA_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\lua.win.*.nupkg" (
    echo [INFO] lua.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] lua.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] lua %LUA_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
