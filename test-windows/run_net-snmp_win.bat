@echo off
:: ============================================================
::  Сборка пакета net-snmp/5.9.4 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_net-snmp_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. net-snmp — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> net-snmp.win.*.nupkg
::  в output-net-snmp-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_net-snmp_win.bat
::  Env:
::    NET_SNMP_VERSION       по умолчанию 5.9.4
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-net-snmp-win
...
if "%NET_SNMP_VERSION%"=="" set NET_SNMP_VERSION=5.9.4
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-net-snmp-win
...
echo [INFO] Ref:      net-snmp/%NET_SNMP_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export net-snmp recipe
echo ============================================
conan export "%ROOT_DIR%\zlib" --version=1.3.1 --no-remote
if errorlevel 1 exit /b 1
conan export "%ROOT_DIR%\openssl" --version=3.4.5 --no-remote
if errorlevel 1 exit /b 1
conan export "%ROOT_DIR%\pcre" --version=8.45 --no-remote
if errorlevel 1 exit /b 1
echo [INFO] conan export net-snmp (%NET_SNMP_VERSION%)
conan export "%ROOT_DIR%\net-snmp" --version=%NET_SNMP_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] net-snmp export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build net-snmp/%NET_SNMP_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building net-snmp/%NET_SNMP_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=net-snmp/%NET_SNMP_VERSION% ^
    -o "pcre/*:build_pcregrep=False" ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] net-snmp %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] net-snmp/%NET_SNMP_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\net-snmp.win.*.nupkg" (
    echo [INFO] net-snmp.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] net-snmp.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] net-snmp %NET_SNMP_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
