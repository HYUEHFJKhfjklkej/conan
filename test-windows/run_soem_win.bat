@echo off
:: ============================================================
::  Сборка пакета soem/2.0.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_soem_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. soem — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> soem.win.*.nupkg
::  в output-soem-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_soem_win.bat
::  Env:
::    SOEM_VERSION       по умолчанию 2.0.0
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-soem-win
...
if "%SOEM_VERSION%"=="" set SOEM_VERSION=2.0.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-soem-win
...
echo [INFO] Ref:      soem/%SOEM_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export soem recipe
echo ============================================
echo [INFO] conan export soem (%SOEM_VERSION%)
conan export "%ROOT_DIR%\soem" --version=%SOEM_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] soem export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build soem/%SOEM_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building soem/%SOEM_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=soem/%SOEM_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] soem %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] soem/%SOEM_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\soem.win.*.nupkg" (
    echo [INFO] soem.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] soem.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] soem %SOEM_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
