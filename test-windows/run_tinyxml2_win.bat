@echo off
:: ============================================================
::  Сборка пакета tinyxml2/11.0.0 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_tinyxml2_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. tinyxml2 — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> tinyxml2.win.*.nupkg
::  в output-tinyxml2-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_tinyxml2_win.bat
::  Env:
::    TINYXML2_VERSION       по умолчанию 11.0.0
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-tinyxml2-win
...
if "%TINYXML2_VERSION%"=="" set TINYXML2_VERSION=11.0.0
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-tinyxml2-win
...
echo [INFO] Ref:      tinyxml2/%TINYXML2_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export tinyxml2 recipe
echo ============================================
echo [INFO] conan export tinyxml2 (%TINYXML2_VERSION%)
conan export "%ROOT_DIR%\tinyxml2" --version=%TINYXML2_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] tinyxml2 export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build tinyxml2/%TINYXML2_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building tinyxml2/%TINYXML2_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=tinyxml2/%TINYXML2_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] tinyxml2 %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] tinyxml2/%TINYXML2_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\tinyxml2.win.*.nupkg" (
    echo [INFO] tinyxml2.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] tinyxml2.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] tinyxml2 %TINYXML2_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
