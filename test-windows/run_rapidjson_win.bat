@echo off
:: ============================================================
::  Сборка пакета rapidjson/cci.20250205 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_rapidjson_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. rapidjson — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> rapidjson.win.*.nupkg
::  в output-rapidjson-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_rapidjson_win.bat
::  Env:
::    RAPIDJSON_VERSION       по умолчанию cci.20250205
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-rapidjson-win
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-rapidjson-win
...
echo [INFO] Ref:      rapidjson/%RAPIDJSON_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export rapidjson recipe
echo ============================================
echo [INFO] conan export rapidjson (%RAPIDJSON_VERSION%)
conan export "%ROOT_DIR%\rapidjson" --version=%RAPIDJSON_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] rapidjson export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build rapidjson/%RAPIDJSON_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building rapidjson/%RAPIDJSON_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=rapidjson/%RAPIDJSON_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] rapidjson %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] rapidjson/%RAPIDJSON_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\rapidjson.win.*.nupkg" (
    echo [INFO] rapidjson.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] rapidjson.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] rapidjson %RAPIDJSON_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
