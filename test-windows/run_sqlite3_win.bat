@echo off
:: ============================================================
::  Сборка пакета sqlite3/3.53.3 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_sqlite3_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. sqlite3 — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> sqlite3.win.*.nupkg
::  в output-sqlite3-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_sqlite3_win.bat
::  Env:
::    SQLITE3_VERSION       по умолчанию 3.53.3
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-sqlite3-win
...
if "%SQLITE3_VERSION%"=="" set SQLITE3_VERSION=3.53.3
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-sqlite3-win
...
echo [INFO] Ref:      sqlite3/%SQLITE3_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export sqlite3 recipe
echo ============================================
echo [INFO] conan export sqlite3 (%SQLITE3_VERSION%)
conan export "%ROOT_DIR%\sqlite3" --version=%SQLITE3_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] sqlite3 export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build sqlite3/%SQLITE3_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building sqlite3/%SQLITE3_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=sqlite3/%SQLITE3_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] sqlite3 %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] sqlite3/%SQLITE3_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\sqlite3.win.*.nupkg" (
    echo [INFO] sqlite3.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] sqlite3.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] sqlite3 %SQLITE3_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
