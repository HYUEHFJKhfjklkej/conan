@echo off
:: ============================================================
::  Сборка пакета pcre/8.45 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_pcre_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. pcre — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> pcre.win.*.nupkg
::  в output-pcre-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_pcre_win.bat
::  Env:
::    PCRE_VERSION       по умолчанию 8.45
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-pcre-win
...
if "%PCRE_VERSION%"=="" set PCRE_VERSION=8.45
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-pcre-win
...
echo [INFO] Ref:      pcre/%PCRE_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export pcre recipe
echo ============================================
echo [INFO] conan export pcre (%PCRE_VERSION%)
conan export "%ROOT_DIR%\pcre" --version=%PCRE_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] pcre export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build pcre/%PCRE_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building pcre/%PCRE_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=pcre/%PCRE_VERSION% ^
    -o "pcre/*:build_pcregrep=False" ^
        ...
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] pcre %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] pcre/%PCRE_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\pcre.win.*.nupkg" (
    echo [INFO] pcre.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] pcre.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] pcre %PCRE_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
