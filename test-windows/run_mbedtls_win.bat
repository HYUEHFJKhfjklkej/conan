@echo off
:: ============================================================
::  Сборка пакета mbedtls/3.6.6 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_mbedtls_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. mbedtls — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> mbedtls.win.*.nupkg
::  в output-mbedtls-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_mbedtls_win.bat
::  Env:
::    MBEDTLS_VERSION       по умолчанию 3.6.6
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-mbedtls-win
...
if "%MBEDTLS_VERSION%"=="" set MBEDTLS_VERSION=3.6.6
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-mbedtls-win
...
echo [INFO] Ref:      mbedtls/%MBEDTLS_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export mbedtls recipe
echo ============================================
conan export "%ROOT_DIR%\zlib" --version=1.3.1 --no-remote
if errorlevel 1 exit /b 1
echo [INFO] conan export mbedtls (%MBEDTLS_VERSION%)
conan export "%ROOT_DIR%\mbedtls" --version=%MBEDTLS_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] mbedtls export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build mbedtls/%MBEDTLS_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building mbedtls/%MBEDTLS_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=mbedtls/%MBEDTLS_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] mbedtls %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] mbedtls/%MBEDTLS_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\mbedtls.win.*.nupkg" (
    echo [INFO] mbedtls.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] mbedtls.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] mbedtls %MBEDTLS_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
