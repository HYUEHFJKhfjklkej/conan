@echo off
:: ============================================================
::  Сборка пакета glog/0.7.1 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_glog_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. glog — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> glog.win.*.nupkg
::  в output-glog-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_glog_win.bat
::  Env:
::    GLOG_VERSION       по умолчанию 0.7.1
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-glog-win
...
if "%GLOG_VERSION%"=="" set GLOG_VERSION=0.7.1
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-glog-win
...
echo [INFO] Ref:      glog/%GLOG_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export glog recipe
echo ============================================
echo [INFO] conan export glog (%GLOG_VERSION%)
conan export "%ROOT_DIR%\gflags" --version=2.3.0 --no-remote
conan export "%ROOT_DIR%\glog" --version=%GLOG_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] glog export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build glog/%GLOG_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building glog/%GLOG_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=glog/%GLOG_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%" -o "glog/*:with_unwind=False" -o "gflags/*:nothreads=False"
    if errorlevel 1 (
        echo [FAIL] glog %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] glog/%GLOG_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\glog.win.*.nupkg" (
    echo [INFO] glog.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] glog.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] glog %GLOG_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
