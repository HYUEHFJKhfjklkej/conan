@echo off
:: ============================================================
::  Сборка пакета jansson/2.15.1 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_jansson_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. jansson — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> jansson.win.*.nupkg
::  в output-jansson-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_jansson_win.bat
::  Env:
::    JANSSON_VERSION       по умолчанию 2.15.1
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-jansson-win
...
if "%JANSSON_VERSION%"=="" set JANSSON_VERSION=2.15.1
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-jansson-win
...
echo [INFO] Ref:      jansson/%JANSSON_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export jansson recipe
echo ============================================
echo [INFO] conan export jansson (%JANSSON_VERSION%)
conan export "%ROOT_DIR%\jansson" --version=%JANSSON_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] jansson export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build jansson/%JANSSON_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building jansson/%JANSSON_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=jansson/%JANSSON_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] jansson %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] jansson/%JANSSON_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\jansson.win.*.nupkg" (
    echo [INFO] jansson.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] jansson.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] jansson %JANSSON_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
