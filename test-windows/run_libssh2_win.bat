@echo off
:: ============================================================
::  Сборка пакета libssh2/1.11.1 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_libssh2_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. libssh2 — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> libssh2.win.*.nupkg
::  в output-libssh2-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_libssh2_win.bat
::  Env:
::    LIBSSH2_VERSION       по умолчанию 1.11.1
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-libssh2-win
...
if "%LIBSSH2_VERSION%"=="" set LIBSSH2_VERSION=1.11.1
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-libssh2-win
...
echo [INFO] Ref:      libssh2/%LIBSSH2_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export libssh2 recipe
echo ============================================
conan export "%ROOT_DIR%\zlib" --version=1.3.1 --no-remote
if errorlevel 1 exit /b 1
conan export "%ROOT_DIR%\openssl" --version=3.4.5 --no-remote
if errorlevel 1 exit /b 1
echo [INFO] conan export libssh2 (%LIBSSH2_VERSION%)
conan export "%ROOT_DIR%\libssh2" --version=%LIBSSH2_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] libssh2 export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build libssh2/%LIBSSH2_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building libssh2/%LIBSSH2_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=libssh2/%LIBSSH2_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] libssh2 %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] libssh2/%LIBSSH2_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\libssh2.win.*.nupkg" (
    echo [INFO] libssh2.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] libssh2.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] libssh2 %LIBSSH2_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
