@echo off
:: ============================================================
::  Сборка пакета cjson/1.7.19 на Windows MSVC (одиночный пакет).
::  Нативный аналог test-astra/build_cjson_nodocker.sh — на Windows без
::  Docker, собирает прямо на агенте через MSVC. cjson — вне grpc-дерева,
::  без транзитивных deps.
::
::  export -> build Release+Debug -> deployer -> cjson.win.*.nupkg
::  в output-cjson-win\.
::
::  Требуется на агенте: MSVC-toolset под профиль + Conan 2.29.0
::  (сначала test-windows\setup.bat). Запуск без pause.
::
::  Usage:    test-windows\run_cjson_win.bat
::  Env:
::    CJSON_VERSION       по умолчанию 1.7.19
::    PROFILE_NAME      по умолчанию win-v143-x64 (ещё win-v142-x64, win-v142-x86)
::    OUTPUT_DIR        по умолчанию <repo>\output-cjson-win
...
if "%CJSON_VERSION%"=="" set CJSON_VERSION=1.7.19
...
if "%OUTPUT_DIR%"=="" set OUTPUT_DIR=%ROOT_DIR%\output-cjson-win
...
echo [INFO] Ref:      cjson/%CJSON_VERSION%
...
:: -----------------------------------------------------------
echo ============================================
echo  Step 1/3: Export cjson recipe
echo ============================================
echo [INFO] conan export cjson (%CJSON_VERSION%)
conan export "%ROOT_DIR%\cjson" --version=%CJSON_VERSION% --no-remote
if errorlevel 1 ( echo [FAIL] cjson export & set EXITCODE=1 & goto :END )
echo.

:: -----------------------------------------------------------
echo ============================================
echo  Step 2/3: Build cjson/%CJSON_VERSION% Release + Debug
echo ============================================
for %%B in (Release Debug) do (
    echo [INFO] Building cjson/%CJSON_VERSION% build_type=%%B shared=%SHARED%
    conan install --requires=cjson/%CJSON_VERSION% ^
        -pr:h="%PROFILE%" -pr:b="%PROFILE%" ^
        --build=missing --no-remote ^
        -s build_type=%%B ^
        -o "*/*:shared=%SHARED%"
    if errorlevel 1 (
        echo [FAIL] cjson %%B build failed
        set EXITCODE=1
        goto :END
    )
)
echo [OK] cjson/%CJSON_VERSION% built (Release+Debug)
echo.
...
if exist "%OUTPUT_DIR%\cjson.win.*.nupkg" (
    echo [INFO] cjson.win.*.nupkg present in %OUTPUT_DIR%
) else (
    echo [FAIL] cjson.win.*.nupkg missing
    set EXITCODE=1
)
...
if "%EXITCODE%"=="0" (
    echo ============================================
    echo  [DONE] cjson %CJSON_VERSION% Windows OK ^(profile %PROFILE_NAME%^)
    echo ============================================
) else (
